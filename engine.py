...
def safe_str(series: pd.Series) -> pd.Series:
    """astype(str) that actually returns strings.

    pandas 3 preserves missing values through .astype(str) instead of turning them
    into the literal 'nan'. Anything that then does str.join or boolean masking on
    the result blows up. This forces real strings with '' for missing.
    """
    return series.astype("object").where(series.notna(), "").astype(str)


def join_cols(df: pd.DataFrame, cols: list[str], sep: str = " › ") -> pd.Series:
    """Vectorised, NaN-safe row label built from several key columns."""
    cols = [c for c in cols if c in df.columns]
    if not cols:
        return pd.Series([""] * len(df), index=df.index)
    out = safe_str(df[cols[0]])
    for c in cols[1:]:
        out = out.str.cat(safe_str(df[c]), sep=sep)
    return out
...
def load_report(data: bytes, name: str) -> pd.DataFrame:
    ...
    for key in ("campaign", "ad_group", "keyword", "search_term", "ad_label"):
        if key in df.columns:
            mask = safe_str(df[key]).str.strip().str.lower().str.startswith("total")
...
def verdict(df: pd.DataFrame, account_cvr: float, target_roas: float,
    ...
    if d.empty:
        d["verdict"] = pd.Series(dtype="object")
        d["dollars_at_stake"] = pd.Series(dtype="float")
        return d

    res = d.apply(_row, axis=1, result_type="expand")
    d["verdict"] = res[0]
    d["dollars_at_stake"] = res[1].astype(float)
