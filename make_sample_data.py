...
def blankify(rows, col_idx, rate=0.04):
    """Real exports contain empty label cells (deleted entities, PMax rows,
    aggregated segments). Inject them so the loader is tested against them."""
    for r in rows:
        for i in col_idx:
...
preamble("Campaign report", blankify(rows, [2, 3]),
...
preamble("Ad group report", blankify(ag_rows, [1]),
...
preamble("Keyword report", blankify(kw_rows, [1, 3]),
...
for _ in range(4000):
    ...
    st_rows.append([term, random.choice(campaigns), random.choice(["Exact match", "Phrase match", "Broad match"]),
                    ...
                    f"{conv:,.2f}", f"${conv*random.uniform(50,350):,.2f}"])
preamble("Search terms report", blankify(st_rows, [1, 2]),
...
preamble("Ad report", blankify(ad_rows, [1, 2]),
