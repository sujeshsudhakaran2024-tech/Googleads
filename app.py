...
from engine import (  # noqa: E402
    CANON, MONTHLY_MULTIPLIER, aggregate, join_cols, load_report, ngrams,
    pacing_table, safe_str, verdict,
)
...
with tabs[0]:
    ...
    for key, group_keys, label in level_map:
        ...
        v["entity"] = join_cols(v, group_keys)
        frames.append(v)
    ...
    if frames:
        ...
        with colf1:
            pick = st.multiselect("Verdicts", sorted(safe_str(actions["verdict"]).unique()),
                                  default=[v for v in actions["verdict"].unique() if v != "Not enough data"])
        with colf2:
            levels = st.multiselect("Levels", sorted(safe_str(actions["level"]).unique()),
                                    default=sorted(safe_str(actions["level"]).unique()))
        ...
        per_level = per_level.loc[order] if order else per_level

        headline = per_level["Recoverable spend"].iloc[0] if len(per_level) else 0.0
        scale_spend = per_level["Spend in scale rows"].iloc[0] if len(per_level) else 0.0
        ...
        m2.metric("Spend in 'scale' rows", money(scale_spend, sym))
...
with tabs[2]:
    ...
    pick_v = st.multiselect("Filter verdicts", sorted(safe_str(cv["verdict"]).unique()),
                            default=sorted(safe_str(cv["verdict"]).unique()), key="camp_v")
...
with tabs[3]:
    ...
    if "ad_groups" not in loaded:
        ...
        camp_names = sorted(safe_str(agv["campaign"]).unique().tolist()) if "campaign" in agv.columns else []
        camps = st.multiselect("Limit to campaigns", [c for c in camp_names if c],
                               default=[], key="ag_camp")
        if camps:
            agv = agv[safe_str(agv["campaign"]).isin(camps)]
